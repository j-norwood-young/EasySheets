...
test.describe('admin sheets (localStorage + share)', () => {
	test('your sheets section appears after creating a sheet and returning home', async ({ page }) => {
		await page.goto('/');
	...
	test('copy admin link copies full sheet URL', async ({ page, context }) => {
		...
		await dialog.locator('li').filter({ hasText: 'Admin' }).getByRole('button', { name: /Copy link/i }).click();
		await expect(dialog.locator('li').filter({ hasText: 'Admin' }).getByRole('button', { name: 'Copied!' })).toBeVisible();
