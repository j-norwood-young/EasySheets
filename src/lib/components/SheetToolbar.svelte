	...
	>
		...
		>
			...
			<div class="mt-5 space-y-4">
				<div class="space-y-2">
					<label for="filter-col" class="block text-xs font-medium text-zinc-600"
						>Column</label
					>
					<select
						id="filter-col"
						bind:value={filterColId}
						class="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 shadow-sm focus:border-zinc-500 focus:ring-2 focus:ring-zinc-500/20 focus:outline-none"
					>
					...
					{#if isNumber}
						<div class="grid gap-3 sm:grid-cols-2">
							<div class="space-y-2">
								<label
									for="filter-op"
									class="block text-xs font-medium text-zinc-600">Operator</label
								>
								<select
									id="filter-op"
									bind:value={filterOp}
									class="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-zinc-500 focus:ring-2 focus:ring-zinc-500/20 focus:outline-none"
								>
							...
							<div class="space-y-2">
								<label
									for="filter-val"
									class="block text-xs font-medium text-zinc-600">Value</label
								>
								<input
									id="filter-val"
									type="number"
					...
					{:else if isDate}
						<div class="grid gap-3 sm:grid-cols-2">
							<div class="space-y-2">
								<label
									for="filter-op-d"
									class="block text-xs font-medium text-zinc-600">Range</label
								>
								<select
									id="filter-op-d"
									bind:value={filterOp}
						...
						<div class="space-y-4">
							<div class="grid gap-3 sm:grid-cols-2">
								<div class="space-y-2">
									<label
										for="filter-op-s"
										class="block text-xs font-medium text-zinc-600">Match</label
									>
									<select
										id="filter-op-s"
										bind:value={filterOp}
							...
							<label
								class="inline-flex items-center gap-2 text-xs font-medium text-zinc-600"
							>
								<input
									type="checkbox"
