			...
			<div class="w-full max-w-sm rounded-lg bg-white p-6 shadow-lg">
				...
				{:else}
					<h3 class="text-lg font-medium text-zinc-900">
						{editColumnId ? 'Edit column' : 'Add column'}
					</h3>
					<form
						onsubmit={(e) => {
							e.preventDefault();
					...
					>
						<div>
							...
							<input
								...
								class="mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2 focus:border-zinc-500 focus:ring-1 focus:ring-zinc-500 focus:outline-none"
							/>
						</div>
						<div>
						...
						{#if newColType === 'number' || newColType === 'currency'}
							<div>
								...
								<p class="mt-0.5 text-xs text-zinc-500">
									Leave empty for auto (0 for integers, 2 for decimals)
								</p>
							</div>
							<div class="flex items-center gap-2">
								...
								/>
								<label for="col-thousands" class="text-sm font-medium text-zinc-700"
		...
		>
			<div class="w-full max-w-md rounded-lg bg-white p-6 shadow-lg">
				...
				<ul class="mt-4 space-y-3">
					<li
						class="flex items-center justify-between rounded-lg border border-zinc-200 bg-zinc-50/50 px-4 py-3"
					>
