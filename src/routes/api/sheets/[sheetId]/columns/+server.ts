...
export const PATCH: RequestHandler = async ({ params, request }) => {
	...
	const sheetColumns = await db.select({ id: columns.id }).from(columns).where(eq(columns.sheetId, sheetId));
	...
	for (let i = 0; i < order.length; i++) {
		await db.update(columns).set({ order: i }).where(and(eq(columns.id, order[i]), eq(columns.sheetId, sheetId)));
	}
	broadcast(sheetId);
	return json({ ok: true });
};
