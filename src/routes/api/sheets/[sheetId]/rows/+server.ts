...
export const GET: RequestHandler = async ({ params, request, url }) => {
	...
	for (const [columnId, param] of Object.entries(filterParams)) {
		let [op, value] = param.includes(':')
			? param.split(/:(.*)/s).map((s) => s.trim())
			: ['eq', param]
		...
		filteredRows = filteredRows.filter((row) => {
			...
			if (op === 'startsWith')
				return caseSensitive
					? String(val).startsWith(value)
					: String(val).toLowerCase().startsWith(String(value).toLowerCase())
			if (op === 'endsWith')
				return caseSensitive
					? String(val).endsWith(value)
					: String(val).toLowerCase().endsWith(String(value).toLowerCase())
			if (op === 'contains')
				return caseSensitive
...
export const PATCH: RequestHandler = async ({ params, request }) => {
	...
	for (let i = 0; i < order.length; i++) {
		await db
			.update(rows)
			.set({ orderNum: i })
			.where(and(eq(rows.id, order[i]), eq(rows.sheetId, sheetId)))
	}
	broadcast(sheetId)
	return json({ ok: true })
}
