<script lang="ts">
	...
	let columns = $state<
		{
			...
			name: string
			type: string
			format?: import('$lib/formatCell').ColumnFormat | null
			width?: number | null
		}[]
	>([])
	...
	function saveSortToStorage(columnId: string, direction: 'asc' | 'desc') {
		...
		try {
			if (!columnId) {
				...
				window.localStorage.setItem(key, JSON.stringify({ columnId, direction }))
			}
		} catch {
			// best-effort only
		}
	...
	function buildRowsUrl() {
		...
		for (const [colId, param] of Object.entries(filters))
			u.searchParams.set(`filter_${colId}`, param)
		return u.toString()
	}
	...
	async function fetchData() {
		...
		try {
			const [sheetRes, rowsRes] = await Promise.all([
				fetch(`/api/sheets/${sheetId}?token=${encodeURIComponent(token)}`, {
					signal,
					cache: 'no-store',
				}),
	...
	function onCellChange(rowId: string, columnId: string, value: string) {
		const canEditThisRow =
			canEdit ||
			(canAppend && (sessionAddedRowIds.has(rowId) || rowId.startsWith('offline-')))
...
<div class="min-h-screen bg-zinc-50 text-zinc-900">
	<header class="sticky top-0 z-20 border-b border-zinc-200 bg-white">
		<OfflineBanner {connected} pendingCount={pendingOfflineRows.length} />
		<div class="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-4 py-3">
			<div class="flex min-w-0 shrink items-center gap-3">
				<LogoLink />
				<span
					class="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-zinc-200 text-zinc-600"
				>
					<PermissionIcon
						permission={permission as 'read' | 'append' | 'edit'}
						class="text-zinc-600"
					/>
				</span>
				<h1 class="truncate text-lg font-medium text-zinc-800">
					{data.name ?? 'Shared sheet'}
				</h1>
			</div>
	...
	<main class="mx-auto max-w-6xl px-4 py-6">
		...
		{:else}
			<DataTable
				...
				editable={canEdit
					? connected
					: canAppend && !connected && pendingOfflineRows.length > 0}
				{editableRowIds}
				editingRowIds={editableRowIds != null
					? connected
						? editingRowIds
						: new Set(pendingOfflineRows.map((p) => p.tempId))
					: undefined}
